...
# Build an experimental Raspberry Pi VyOS A/B image from:
...
#   no commit yet; normal reboot therefore returns to A
#
# This v0.1 builder NEVER writes to a physical SD/eMMC/NVMe device. It only
# creates and modifies OUT_IMG and opens the Armbian source image read-only.
#
# Usage:
#   sudo ./scripts/pi5/build-vyos-pi-ab-image.sh \
#     /path/to/armbian.img.xz \
#     /path/to/vyos-rootfs-pi5-merged.tar.gz \
#     /path/to/vyos-rpi-ab-test.img
#
# Optional environment variables:
#   AB_IMAGE_SIZE_GIB=13   final image size, integer GiB (default: 13)
...
AB_IMAGE_SIZE_GIB="${AB_IMAGE_SIZE_GIB:-13}"
...
for cmd in xz truncate stat losetup lsblk blkid parted mkfs.vfat mkfs.ext4 \
           ...
           sync python3 find rm mkdir mv cp tr sed; do
...
for n in "$AB_IMAGE_SIZE_GIB" "$AB_CONTROL_MIB" "$AB_BOOT_MIB" "$AB_ROOT_MIB"; do
    [[ "$n" =~ ^[0-9]+$ ]] || fail "image/partition sizes must be positive integers"
done

(( AB_IMAGE_SIZE_GIB >= 1 )) || fail "AB_IMAGE_SIZE_GIB must be >= 1"
...
TOTAL_MIB=$(( AB_IMAGE_SIZE_GIB * 1024 ))
P4_END=$(( TOTAL_MIB - 1 ))

(( P6_END + 16 < P4_END )) || {
    fail "AB_IMAGE_SIZE_GIB=${AB_IMAGE_SIZE_GIB} is too small for the requested A/B slots (p6 ends at ${P6_END} MiB)"
}
...
create_output_layout() {
    echo "==> Creating ${AB_IMAGE_SIZE_GIB} GiB sparse A/B image"
    truncate -s "${AB_IMAGE_SIZE_GIB}G" "$IMG"
...
echo "==> VyOS Raspberry Pi A/B image builder v0.1"
echo "    Armbian base : $ARMBIAN_IMAGE"
echo "    Merged rootfs: $MERGED_TAR"
echo "    Output       : $OUT_IMG"
echo "    Image size   : ${AB_IMAGE_SIZE_GIB} GiB"
...
echo
echo "==> Raspberry Pi VyOS A/B test image complete"
echo "    Image : $OUT_IMG"
echo "    Size  : $(stat -c '%s' "$OUT_IMG") bytes"
echo "    SHA256: $FINAL_SHA"
echo "    Normal boot      : slot A (p2 -> p5)"
echo "    One-shot tryboot : slot B (p3 -> p6)"
echo "    Commit/healthcheck/watchdog are intentionally NOT implemented in v0.1"
